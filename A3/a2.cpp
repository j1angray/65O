...
bool BreadthFirst_search(int srce, int dest, int v, vector<int> adj_list[], int pred[]) // Ref: BFS algorithm
{
...
void get_cmd(vector<int> *&adj)
{
    ...
    while(cin >> cmd)
    {
        if (cmd == 'V') {
            int a;
            cin >> a;
            cout << "V " << a << std::endl; //for a3_getln
            if (a > 0){
                v = a;
            }
            else{
        ...
        if (cmd == 'E') {
            cin >> input_edges;
            cout << "E " << input_edges << endl; //for a3_getln
